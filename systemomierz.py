...
class dimensionsIndexingDictionaries:
       ...
       for aktualnaKrotka in listaKrotekWejsciowych:
           ...
           for indeksSlownika in range(self.liczbaWymiarowIndeksowaniaKomorekTensoraWeWy):
               ...
               if(aktualnyKlucz not in self.listaSlownikowIndeksowania[indeksSlownika].keys()): 
                   self.listaSlownikowIndeksowania[indeksSlownika][aktualnyKlucz]=len(self.listaSlownikowIndeksowania[indeksSlownika].keys())+1
   ...
   def getIndex(self, numerSlownika, kluczAdresujacyIndeks):
       if(kluczAdresujacyIndeks not in self.listaSlownikowIndeksowania[numerSlownika].keys()): 
           self.listaSlownikowIndeksowania[numerSlownika][kluczAdresujacyIndeks]=len(self.listaSlownikowIndeksowania[numerSlownika].keys())+1
...
class arrayBasedDataRepresentation:
    ...
    def __init__(self, dids, trainSetTuplesList=[]):
       ...
       self.trainsetDataRepresentation,self.rowSumsVector,self.columnSumsVector=self.storeData(dids,trainSetTuplesList)
...
class recSystem:
    ...
    def __init__(self, dids1, inputArray, systemType):
     ...
     #   print(inputArray)
        self.dids1=dids1
        self.numberOfDimensionsToBeLeft=min(dids1.liczbaWymiarowPrzestrzeniOdpowiadajacejDanemuKierunkowiIndeksowania[1:])
    ...
    def GetSystemName(self):
        ...
        if self.typeOfSystem == 3:
            return "classical-averages-based"
        if self.typeOfSystem == 4:
            return "overallCentring-based_MPCA"
        if self.typeOfSystem == 5:
            return "statisticalCentring-based_MPCA with decentring"
        if self.typeOfSystem == 6:
            return "statistical-averaged-based"
...
#            #print("k: ",k)
        ...
        if (self.typeOfSystem==4):
            ...
            self.columnAveragesMatrix=np.outer(np.ones(len(self.rowSumsVector)),self.columnAveragesVector)
            self.rowAveragesMatrix=np.outer(self.rowAveragesVector, np.ones(len(self.columnSumsVector)))
            self.averagesBasedDataRepresentation=(self.columnAveragesMatrix+self.rowAveragesMatrix)-self.overallAverageScalar
            ...
            for i in range(len(self.rowSumsVector)):
                for j in range(len(self.columnSumsVector)): 
                    if self.trainsetDataRepresentation[i+1,j+1] != 0:              
                        self.coreDataRepresentation[i,j]=self.trainsetDataRepresentation[i+1,j+1]-self.rowAveragesVector[i]-self.columnAveragesVector[j]+self.overallAverageScalar
...
#                queryResult=0
        ...
        if (self.typeOfSystem==2)or(self.typeOfSystem==3)or(self.typeOfSystem==4)or(self.typeOfSystem==5)or(self.typeOfSystem==6):
...
#            #print(self.processedDataRepresentation[self.dids.listaSlownikowIndeksowania[1][queryTuple[0]]-1,self.dids.listaSlownikowIndeksowania[2][queryTuple[1]]-1])
            if (queryTuple[0] in self.dids1.listaSlownikowIndeksowania[1].keys())and(queryTuple[1] in self.dids1.listaSlownikowIndeksowania[2].keys()):
                queryResult=self.processedDataRepresentation[self.dids1.listaSlownikowIndeksowania[1][queryTuple[0]]-1,self.dids1.listaSlownikowIndeksowania[2][queryTuple[1]]-1]
            else:
                queryResult=0
...
#                    #print('a new item case!  ',end="",flush=True)
    ...
    def getMultiQueryFloatResults(self, queryTuplesList):
      ...
      #          samplingDimensionalityVector.append(defaultNumberOfDimensions)    
            ...
            self.aMOPCASystem=MOPCA.MOPCASystem(trainSet,self.samplingDimensionalityVector,kolejka,DIDsQueue,monotensorOutputMicroTuples,aManager,YAMFDict,DIDsDict) 
...
def getPrecisionVsRecallPointsForASingleCurve(multiQueryBinaryResults):
    ...
    for currentPvsRCurvePoint in range(numberOfPvsRCurvePoints):
        ...
        for currentMultiQueryResultTupleNumber in range(len(multiQueryBinaryResults[currentPvsRCurvePoint])):
            if ((multiQueryBinaryResults[currentPvsRCurvePoint][currentMultiQueryResultTupleNumber][0]==1)and(testSet[currentMultiQueryResultTupleNumber][0]==1)):
                TP+=1
            if ((multiQueryBinaryResults[currentPvsRCurvePoint][currentMultiQueryResultTupleNumber][0]==-1)and(testSet[currentMultiQueryResultTupleNumber][0]==-1)):
                TN+=1
            if ((multiQueryBinaryResults[currentPvsRCurvePoint][currentMultiQueryResultTupleNumber][0]==1)and(testSet[currentMultiQueryResultTupleNumber][0]==-1)):
...
def getTPVsPPointsForASingleCurve(multiQueryBinaryResults):
    ...
    for curvePoint in range(numberOfCurvePoints):
        ...
        for currentMultiQueryResultTupleNumber in range(len(multiQueryBinaryResults[curvePoint])):
            if ((multiQueryBinaryResults[curvePoint][currentMultiQueryResultTupleNumber][0]==1)and(testSet[currentMultiQueryResultTupleNumber][0]==1)):
                TP+=1
...
def makeCurveFigure(horizontalValues, verticalValues, numberOfRandomTrainAndTestSetSplitCases, numberOfTRSteps, numberOfRecSystems, recSystemsLabels, horizontalAxisLabel, verticalAxisLabel, fileName, legendLocation):
    plt.ioff()
    plt.figure(figsize=(20,10),linewidth=0.1)

    hsv = plt.get_cmap('hsv')
    colors = hsv(np.linspace(0, 1.0, numberOfRecSystems + 1))
    ...
    for randomTrainAndTestSetSplitCaseNumber in range(numberOfRandomTrainAndTestSetSplitCases):
       for currentTRStepNumber in range(numberOfTRSteps):
           ...
           plt.subplot(numberOfRandomTrainAndTestSetSplitCases+0,numberOfTRSteps+0,currentTRStepNumber+1+((numberOfTRSteps+0)*(randomTrainAndTestSetSplitCaseNumber+0)))    
           for systemNumber in range(numberOfRecSystems):
                ...
                plt.plot(horizontalValues[randomTrainAndTestSetSplitCaseNumber][currentTRStepNumber][systemNumber],verticalValues[randomTrainAndTestSetSplitCaseNumber][currentTRStepNumber][systemNumber], marker='o',markersize=1,label=recSystemsLabels[systemNumber], color=colors[systemNumber])
                axes = plt.gca()
                axes.spines['left'].set_linewidth(0.2)
                axes.spines['right'].set_linewidth(0.2)
                axes.spines['bottom'].set_linewidth(0.2)
...
#                axes.set_xlim([0,1])
#                axes.set_ylim([0,1])
                title="randomTrainAndTestSetSplitCase#"+str(randomTrainAndTestSetSplitCaseNumber)+", trainingRatio="+str(round(trainingRatio,2))
    #            plt.title(title)
                plt.title(title, fontsize=6, y=1.03)
...
if __name__ == '__main__':
    ...
    from wlasnyrs import SSPcossim, SSPpearson, SSpearson_enrich
...
#    numberOfRecSystems=3
    ...
    #    #print()
        ...
        for currentTRStepNumber in range(numberOfTRSteps):
            ...
            trainingRatio=round((currentTRStepNumber+1)*trainingRatioStep,10)
    #        #print()
            print 'currentTRStepNumber: ',currentTRStepNumber
...
#            sortedColumnSums[-1].append(sortedColumnSumsVector)
    ...
    #        
            ...
            rs = OwnRecSystem(dids1, copy.deepcopy(abdr1.trainsetDataRepresentation))
            recSystems.append(rs)

            rs = SSPpearson(dids1, copy.deepcopy(abdr1.trainsetDataRepresentation))
            rs.trainSet = trainSet
            recSystems.append(rs)

            rs = SSpearson_enrich(dids1, copy.deepcopy(abdr1.trainsetDataRepresentation))
            rs.trainSet = trainSet
            recSystems.append(rs)
...
#            recSystems.append(rs)       
         ...
         #   recSystems.append(prepareMultiSvdRS(10,trainSet, True,False))
            ...
            numberOfRecSystems = len(recSystems)

            recSystemsLabels = []
            for rs in recSystems:
                recSystemsLabels.append(rs.GetSystemName())
    ...
    #            currentRecSystem.provideTrainset(abdr)
                ...
                multiQueryBinaryResults,multiqueryFloatResults_=getMultiQueryBinaryResults(currentRecSystem,tso, numberOfPvsRCurvePoints)
                precisionPoints,recallPoints=getPrecisionVsRecallPointsForASingleCurve(multiQueryBinaryResults)
                
                altPrecisionPoints,altRecallPoints=altGetPrecisionVsRecallPointsForASingleCurve(multiqueryFloatResults_,testSet,20)                
...
#    recSystemsLabels.append('MOPCADraftSHOOI')
    ...
    for randomTrainAndTestSetSplitCaseNumber in range(numberOfRandomTrainAndTestSetSplitCases):
        for currentTRStepNumber in range(numberOfTRSteps):
            ...
            plt.subplot(numberOfRandomTrainAndTestSetSplitCases+0,numberOfTRSteps+0,currentTRStepNumber+1+((numberOfTRSteps+0)*(randomTrainAndTestSetSplitCaseNumber+0)))    
            for systemNumber in range(numberOfRecSystems):
                plt.plot(recallValues[randomTrainAndTestSetSplitCaseNumber][currentTRStepNumber][systemNumber],precisionValues[randomTrainAndTestSetSplitCaseNumber][currentTRStepNumber][systemNumber], marker='o',markersize=3,label=recSystemsLabels[systemNumber], color=colors[systemNumber])
                axes = plt.gca()
                axes.spines['left'].set_linewidth(0.2)
                axes.spines['right'].set_linewidth(0.2)
                axes.spines['bottom'].set_linewidth(0.2)
    ...
    makeCurveFigure(PValues, TPValues, numberOfRandomTrainAndTestSetSplitCases, numberOfTRSteps, numberOfRecSystems, recSystemsLabels, 'number of Positives', 'number of True Positives', 'PositivesVsTruePositives',0)
    PvsTPFigureData=(PValues, TPValues, numberOfRandomTrainAndTestSetSplitCases, numberOfTRSteps, numberOfRecSystems, recSystemsLabels, 'number of Positives', 'number of True Positives', 'PositivesVsTruePositives',0)


    makeCurveFigure(recallValues, precisionValues, numberOfRandomTrainAndTestSetSplitCases, numberOfTRSteps, numberOfRecSystems, recSystemsLabels, 'Recall', 'Precision', 'PrecisionVsRecall_2',0)

    PvsRFigureData=(recallValues, precisionValues, numberOfRandomTrainAndTestSetSplitCases, numberOfTRSteps, numberOfRecSystems, recSystemsLabels, 'Recall', 'Precision', 'PrecisionVsRecall_3',0)


    makeCurveFigure(altRecallValues,altPrecisionValues, numberOfRandomTrainAndTestSetSplitCases, numberOfTRSteps, numberOfRecSystems, recSystemsLabels, 'Recall', 'Precision', 'PrecisionVsRecall_4',0)
